// (c) 2008-10 Live365, Inc. - Custom PRO Player generic include file
//
//******************************************************************************************************************
...
//
// Default to WMP on IE; FLASH2 everywhere else.  Use "poc=8" to set to FLASH3.
//
var poc = (navigator.platform.indexOf("Win32") != -1 && navigator.appName.indexOf("Microsoft") != -1) ? 2 : 7;
...
function GetLaunchURL()
{
	...
	if (player == '365' || player == 'WMP' || player == 'FLASH' || player == 'FLASH3')
		launchUrl += "/play/";
	else {
...
function HandleAction(action)
{
	...
	if (action == 'play')			// launch embedded player control after delay
		 gOnLoadTimer = window.setTimeout("DoOnLoadActions()", 1000);	// workaround Firefox DOM issues
	else {
		if (player != "MP3") {
			if (action == 'playnow') {// launch embedded player control immediately
				Play(launchUrl);
				UpdatePlayButton('playnow');
			}
			else if (action == 'stop') {
				Stop();
...
//	s += '<img border="0" src="/images/dot.gif" width="81" height="' + gPreBtnVertSpace + '"><br>';
	s += '<img name="button_play" alt="PLAY" src="' + imageDir + 'radio-play.gif" width="41" border="0" height="36" style=" margin-top: 5px; margin-left:30px; cursor:pointer; cursor:hand" onclick="HandleAction(\'playnow\')">';


/* play button is both stop/pause and play!

	if (player != 'MP3')
		s += '<img name="button_stop" alt="STOP" src="' + imageDir + 'radio-stop.gif" width="45" border="0" height="36" style="cursor:pointer; cursor:hand" onclick="HandleAction(\'stop\')">';
...
function GetPlayURL()
{
	return Nvl('streamUrl', '');
}

function GetPlayer()
{
	return Nvl('player', 'FLASH');
}
...
function UpdatePlayButton(action)
{
	...
	if(action == 'playnow') {
	//if we're playing, make the button a pause button
	  doc.images["button_play"].src = gSiteImageDir + "radio-pause.gif";
	  doc.images["button_play"].onclick = function() { HandleAction('stop') };
	} else { 
	//if we're paused, make the button a play button
	  doc.images["button_play"].src = gSiteImageDir + "radio-play.gif";
	  doc.images["button_play"].onclick = function() { HandleAction('playnow') };
	}
}
