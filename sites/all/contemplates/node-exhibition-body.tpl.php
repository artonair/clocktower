	...
	<div class="header"> 
		...
		<div class="info">
				...
				if($node->type == "residency") { ?>
	      	    <h6 class="type"><span><?php print l("Residencies", "projects/residencies"); ?></span></h6>
				...
				if($node->type == "exhibition") { ?>
	      	    <h6 class="type"><span><?php print l("Exhibitions", "projects/exhibitions"); ?></span></h6>
	   	      	<?php if($node->field_dates[0]['value']) { ?>
	    			<h6 class="dates">
	    		  <?php if($node->field_dates[0]['value'] == $node->field_dates[0]['value2']) print '';  else print ''; ?>
	      			<!--<span class="exhibited_date_itself"><?php //print $node->field_dates[0]['view']; ?></span>-->
	      			...
	      			&nbsp;-&nbsp;<?php print format_date(strtotime($node->field_dates[0]['value2']), 'custom', 'F d, Y',0); ?>
	      			</span></h6><!-- /.dates -->
	    			<?php } else { ?>
	      				<h6 class="dates">
    		...
    		<?php if(!empty($node->field_partner_venue[0]['view'])) { ?>    	  
    	  	<div class="map-icon"><i class="icon icon-location"></i></div>
    	  <h6 class="venue"><?php $artistcount = 0;
			...
			<h6 class="venue">At <?php print $node->field_venue[0]['view']; ?>
			<?php if($node->field_map_link[0]['value']) { ?>
   	...
   	<?php } else { ?>
          <?php if($node->field_venue[0]['value']) { ?>
			<div class="map-icon"><i class="icon icon-location"></i></div><h6 class="venue"><?php print $node->field_venue[0]['view']; ?>
			<?php if($node->field_map_link[0]['value']) { ?>
			<span><a href="<?php print $node->field_map_link[0]['safe']; ?>" target="_blank">Map</a></span>
			<?php } ?>
			</h6><!-- /.venue -->
